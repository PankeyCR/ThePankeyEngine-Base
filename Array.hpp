...
#if defined(Array_LogApp) && defined(pankey_Log)
...
#if defined(PointerArray_LogApp) && defined(pankey_Log)
	#include "Logger_status.hpp"
	#define PointerArrayLog(status,method,mns) pankey_Log(status,"Array",method,mns)
#else
	#define PointerArrayLog(status,method,mns)
#endif
...
class Array{
	...
	protected:
		...
		virtual T* createFilledArray(int a_size){
			...
			PointerArrayLog(pankey_Log_EndMethod, "createFilledArray", this->m_t_value);
	...
	public:
		...
		virtual T* clonePointer(){
			...
			PointerArrayLog(pankey_Log_Statement, "clonePointer", this->m_t_value);
			...
			PointerArrayLog(pankey_Log_Statement, "clonePointer", "new pointer");
			PointerArrayLog(pankey_Log_Statement, "clonePointer", cloneArray);
		...
		virtual Array<T> insertLocalArrayPointer(int a_position, const T* a_pointer, int a_size){
			...
			PointerArrayLog(pankey_Log_Statement, "insertLocalArrayPointer", a_pointer);
			...
			if(this->isEmpty()){
				...
				//ArrayLog(pankey_Log_Statement, "insertLocalArrayPointer", "Array Size:");
				//ArrayLog(pankey_Log_Statement, "insertLocalArrayPointer", this->getSize());
				PointerArrayLog(pankey_Log_Statement, "insertLocalArrayPointer", this->m_t_value);
				//ArrayLog(pankey_Log_EndMethod, "insertLocalArrayPointer", "");
				return *this;
			}
			this->expandIfNeeded(i_array_length);
